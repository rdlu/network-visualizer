...
<?=
Form::open(Request::current()->controller()
    . '/setup/' . $sourceEntity->id, array('id' => 'newEntity', 'class' => 'bForms')) ?>
...
<fieldset title="Dados da Sonda de Destino">
    ...
    <img src="<?=url::site('images/boardMenu/destination.png')?>" alt="Sonda de Destino"
...
<fieldset title="Dados do Perfil a ser utilizado">
    ...
    <div style="float:left;margin-right:15px; height:80px;"><img src="<?=url::site('images/boardMenu/profiles.png')?>"
                                                                 alt="Sonda de Destino" style="vertical-align:middle;"/>
    ...
    <ul class="nice" id="metricas">
        ...
        <li class="nice checkbox">
            <input type="checkbox" id="metrics[<?=$key?>]" name="metrics[]" class="nice medium"
                   value="<?=$metric->id?>" <?=($metric->order > 6) ? '' : 'checked=checked'?>>
        ...
        <ul id="limiares" class="nice checkbox">
            ...
            <li class="nice checkbox" style="padding:8px;"><?=$mName?>:
                <?php if ($tValues['reverse']): ?>
                    <span style="color:#336600;"><?=$tValues['max']?></span> <span
                        style="color:#993b3b"><?=$tValues['min']?></span>
                    <?php else: ?>
                    <span style="color:#993b3b"><?=$tValues['min']?></span> <span
                        style="color:#336600;"><?=$tValues['max']?></span>
                    <?php endif;?>
            </li>
            <?php endforeach; ?>
        </ul>
